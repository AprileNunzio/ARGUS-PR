...
Ultimo aggiornamento: 2026-09-05 · versione progetto: 0.44.1
...
| DELETE | `/api/people/all` | `camera.manage` |
| POST | `/api/people/:id/merge` | `camera.manage` |
| POST | `/api/people/match` | `live.view` |
| POST | `/api/people/extract-face` | `camera.manage` |
| GET | `/api/people/logs/faces` | `live.view` |
| GET | `/api/people/logs/:id` | `live.view` |
| POST | `/api/people/logs/:id/correct` | `camera.manage` |
| DELETE | `/api/people/logs/:id` | `camera.manage` |
| DELETE | `/api/people/logs/all` | `camera.manage` |
...
Aggiunte della versione 0.44.1: **corretto un `ReferenceError` che rompeva la scheda persona** (`observations` letto prima della sua dichiarazione: la pagina non si apriva affatto). Rimosso codice morto: `web/features/people/people_form.js` (157 righe mai importate da nessuno), l'export inutilizzato `calculateBiometricMetrics` in `face_matcher.js` (superato da `deriveBiometrics` in `face_geometry.js`), e gli export `ROLE_CHIPS` e `sampleWeight` resi locali. Tolti i **cache-buster `?v=0.36.4`** da `index.html` e `wall.html`: erano fermi a otto versioni prima e non servivano, perche' gli asset escono gia' con `Cache-Control: no-cache` e il cambio di versione a caldo e' gestito da `version_watch.js`.

Aggiunte della versione 0.44.0: **catalogo persone e volti riscritto**. Il riconoscimento non creava mai una corrispondenza perche' l'arruolamento falliva in silenzio: `execFile` rigetta quando `worker.py` esce con codice 1, quindi ogni esito `ok:false` (tipicamente "nessun volto rilevato" su un ritaglio gia' stretto) diventava un 500 e il profilo veniva salvato **senza vettore**. Ora l'estrazione vive in `face_enroll_service.js`, legge lo stdout anche in caso di uscita non zero e restituisce un errore di validazione leggibile; `vision_enroll.py` ritenta con ingrandimento e bordo replicato, ritaglia con margine e produce una miniatura 256px. **I face_logs conservano l'embedding** (migrazione 021): creare un profilo da un transito riusa quel vettore invece di ri-analizzare il ritaglio, e **associare un transito a una persona esistente insegna al profilo** (centroide mobile, galleria, contatore campioni). **Il modello 3D si riadatta alla persona a ogni avvistamento**: `face_geometry.js` mantiene una media pesata delle proporzioni facciali con piu' peso ai fotogrammi frontali e nitidi, e `face_mesh_model.js` genera da quelle proporzioni un busto denso (circa 2.700 vertici, sezioni superellittiche, rilievo di naso, arcate, zigomi, labbra e mento) reso in `face_mesh_render.js` con back-face culling, ordinamento per profondita' e illuminazione lambertiana; il pannello e' orbitabile col trascinamento. **Selettore persona con ricerca** al posto delle tendine: associazione e fusione passano da `person_picker.js`. **Elimina tutti** per catalogo e transiti. Corretti inoltre: **il riconoscimento facciale non si lasciava attivare** (dipende da "Volti", che il server spegneva in silenzio: ora la UI accende il prerequisito e lo dichiara), **il caricamento immagine dallo scanner non funzionava** (l'handler leggeva un input mai inserito nel DOM), **`confirmPanel` ignorava il parametro `body`** (nessuna tendina compariva mai nei pannelli di conferma), **cinque attributi `style` inline bloccati dalla CSP**, **quattro nomi di icona inesistenti**, e **cinque token CSS mai definiti** (`--foreground`, `--surface-1`, `--radius-md`, `--danger`, `--good`) che lasciavano mezza interfaccia senza sfondi e senza angoli arrotondati.
