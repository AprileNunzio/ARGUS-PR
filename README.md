...
> **Stato attuale: 1.0.0.** Release inaugurale pronta per la produzione. Registrazione continua con catena di integrità crittografica, rilevamento di persone, veicoli e animali, biometria dei volti conforme al GDPR, ANPR, automazioni con relè e varchi, planimetrie con barriere virtuali, controllo PTZ, supporto multilingua (Italiano/Inglese), accelerazione GPU per la decodifica e per l'inferenza, e installatori autonomi per Windows e Linux. Vedi [Roadmap](#roadmap).
...
| **Archivio navigabile** | Linea temporale ingrandibile fino a sei secondi: clicchi l'istante e parte la riproduzione da lì |
...
| **Internazionalizzazione (i18n)** | Cataloghi linguistici modulari BCP 47 (`it`, `en`), cambio lingua al volo nel setup, nelle impostazioni e nella topbar, lazy-loading dei moduli e protezione Zero-Trust da path traversal |

### In sviluppo

Lettura targa dal flusso a piena risoluzione · Analisi differita dei segmenti registrati · Descrizione leggibile degli eventi (transito o sosta)
...
| `ARGUS_PLATE_FORMAT` | `italian` | Forma della targa usata per validare e correggere le letture ANPR |
...
| **F6** | Planimetria, barriere virtuali, PTZ e ronde, notifiche Telegram/MQTT, relè fisici | ✅ completata |
| **F8** | Prova sul campo: correzione della decodifica YOLOX, telecamere panoramiche, registrazione con audio PCM, ANPR su targhe ruotate | ✅ completata |
| **F9** | Lettura targa dal flusso a piena risoluzione e analisi differita dei segmenti registrati | 🔜 in corso |
...
Le sorgenti ultra-larghe (panoramiche 32:9 e simili) ricevono un fotogramma di analisi più largo e vengono analizzate a finestre quadrate sovrapposte: un 32:9 dentro l'ingresso quadrato della rete lascerebbe due terzi di tela vuota. La risoluzione della sorgente viene rilevata da sola con ffprobe. Ogni soglia geometrica è espressa **in rapporto al fotogramma, mai in pixel assoluti**, perché non si può sapere quale telecamera installerà l'utente finale.
...
<details>
<summary><b>La telecamera incide data e ora nel video e l'ANPR inventa targhe</b></summary>

Una scritta incisa dalla telecamera — data, ora, nome della via — è **strutturalmente indistinguibile da una targa**: `09:17:21`, tolti i due punti, sono sei cifre allineate, compatte e della stessa altezza. Supera ogni filtro geometrico.

Le difese attive sono il voto su letture concordi e la validazione contro forme di targa reali, che scartano la grande maggioranza di questi casi. La soluzione pulita è **spegnere la sovrimpressione sulla telecamera**. Toglie anche i volti fantasma che una scritta su sfondo color incarnato può generare.
</details>
...
Le pull request sono benvenute. I vincoli architetturali del progetto sono stretti e applicati con rigore: Clean Architecture, niente commenti nel codice, nessun file oltre 500 righe, nessuna dipendenza aggiunta senza necessità dimostrata, CSP mai indebolita.

Due regole imparate sul campo, da conoscere prima di toccare il codice di visione o registrazione:

- **Nessuna soglia geometrica in pixel assoluti.** Non si può sapere quale telecamera installerà l'utente finale: una costante in pixel cambia significato al variare della risoluzione. Esprimila come frazione del fotogramma.
- **Una funzione di visione o registrazione è viva solo se provata su un fotogramma vero.** Il rilevamento oggetti non ha prodotto nulla per decine di versioni, e la registrazione non ha mai scritto un file con audio PCM: entrambi con la suite verde, perché i test coprivano la forma dei dati e non il fatto che i dati arrivassero. Una sorgente sintetica prova il protocollo, non il riconoscimento.
