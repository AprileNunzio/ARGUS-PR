...
> **Stato di sicurezza (v1.0.0).** **§1 MFA TOTP** è operativo (`src/security/totp.js`, `test/totp.test.js` e `test/auth_mfa.test.js`); **§3 catene di integrità** è operativo per i segmenti video (`segment_index.js`, con `sha256`, `prevHash` e `chainHash` per ogni segmento) e per il registro di audit (`src/security/audit.js`).
>
> I capitoli §1 e §3 illustrano l'architettura delle difese implementate.

Principi di sicurezza e standard di implementazione per ARGUS-PR.

Ogni sezione specifica: **cosa costruire**, **con quali primitive presenti nel progetto**, e **come dimostrare che funziona**. Nessuna dipendenza esterna: tutto il necessario risiede in `node:crypto` e in `src/security/`.
...
**Firmare ogni tag** — `git tag -s vX.Y.Z -m "..."`, poi `git push origin vX.Y.Z`. Aggiungi la firma al ciclo di rilascio ufficiale: un tag non firmato, da quel momento, è un tag che gli impianti rifiuteranno.
...
Quando ne completi una: entrambe le suite di test verdi, documentazione allineata nello stesso commit, tag firmato, e release ufficiale compilata.
