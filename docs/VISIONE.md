...
> **Stato al 2026-09-06 (v0.47.2): costruito, provato sul campo, con limiti noti.** `src/features/vision/` e `vision/` contengono rilevamento oggetti, tracciamento IoU, ANPR con voto su fotogrammi multipli, e biometria YuNet + SFace con soglia 0.363. L'ingresso macchina `POST /api/detections` con chiavi ad hash SHA-256 è attivo.
>
...
>
> ### Cosa ha insegnato la prima telecamera vera
>
> Fino alla 0.45.0 tutte le verifiche usavano una sorgente sintetica generata con ffmpeg, e **passavano**. Al primo collegamento di una telecamera IP reale, il 6 settembre 2026, sono emersi due difetti che nessun test sintetico poteva vedere:
>
> - **Il rilevamento oggetti non aveva mai prodotto una rilevazione.** Gli ONNX YOLOX pubblicati da Megvii escono *non decodificati*: le prime quattro colonne sono scostamenti di griglia, non pixel. Venivano usati come coordinate, ogni riquadro cadeva sotto la soglia minima e spariva. Silenzio totale, nessun errore. Corretto con `decode_yolox()`, che riconosce da sé se il modello è già decodificato e in quel caso non tocca nulla.
> - **La registrazione non scriveva alcun file** quando la telecamera manda audio PCM, perché l'MP4 non può contenere `pcm_mulaw`. Corretto convertendo l'audio in AAC e lasciando il video in copia.
>
> La lezione, valida oltre questi due casi: **una sorgente sintetica prova il protocollo, non il riconoscimento.** Un fotogramma generato da ffmpeg ha statistiche che nessuna scena reale ha. Prima di dichiarare affidabile una funzione di visione, serve un fotogramma vero.
>
> ### Cosa è provato e cosa no, oggi
>
> | | esito misurato |
> |---|---|
> | Rilevamento oggetti, immagini di riferimento | `bus 0.94`, `person 0.88`, `dog 0.86` |
> | Rilevamento veicoli, panoramica 3840x1080 quasi zenitale | 0.71 con YOLOX-tiny a 1920x540 a finestre; **YOLOX-nano non basta** su questa geometria |
> | Lettura targa, fotogramma di analisi 1920x540 | illeggibile |
> | Lettura targa, fotogramma nativo 3840x1080 | sei caratteri su sette |
> | Sovrimpressione incisa dalla telecamera | **indistinguibile da una targa**: vedi §6 |
>
> Restano non provati: PTZ su hardware reale, ONVIF su marche diverse da quella collaudata, e il comportamento notturno in infrarosso.
...
> **Aggiornato alla v0.46.0, dopo la prova sul campo.** I quattro punti qui sotto restano validi come impostazione, ma la pratica ha aggiunto tre vincoli che non erano previsti e che sono costati altrettanti difetti in produzione. Leggili prima del resto.
>
> **a) I caratteri devono formare una sola riga compatta e attaccata.** La prima versione raccoglieva ogni contorno simile a un carattere nel ritaglio, li ordinava per ascissa e li incollava: prendeva una lettera da una parte dell'immagine e una cifra da un'altra, producendo stringhe mai esistite. Ora `glyph_boxes()` estrae i candidati e `plate_glyph_line()` tiene solo quelli di altezza simile, allineati su una **retta interpolata ai minimi quadrati** — non una base orizzontale, perché la targa è quasi sempre inclinata — e separati da spazi non superiori a una larghezza di carattere. Il gruppo deve contenere fra quattro e nove caratteri. L'OCR riceve **solo il rettangolo di quel gruppo**, raddrizzato da `deslant_line()`.
>
> **b) La targa può essere ruotata di 90 gradi.** Su una panoramica quasi zenitale l'auto è di traverso e la targa si legge dall'alto in basso. Un filtro sul solo `cv2.boundingRect` non può vederla: serve `cv2.minAreaRect` più una trasformazione prospettica (`upright_patch()`), e la chiusura morfologica va fatta **sia orizzontale sia verticale**.
>
> **c) Non fabbricare mai una confidenza.** La prima versione restituiva `0.85` fisso per qualunque uscita del CRNN. Quel numero, che sembrava alto, superava ogni soglia e ha scritto decine di targhe inventate nel registro accessi. Se il riconoscitore non fornisce un punteggio, dillo: `PLATE_UNSCORED_CONFIDENCE` esiste per questo, e la decisione la prendono il formato e il voto.
>
> **Limite che non si risolve nel codice**: una sovrimpressione incisa dalla telecamera è indistinguibile da una targa. `09:17:21`, tolti i due punti, sono sei cifre allineate, compatte e della stessa altezza: supera qualunque filtro geometrico. Va spenta sulla telecamera o mascherata per telecamera.
...
> **Trappola verificata in produzione.** Lo schema qui sotto contiene un errore che è rimasto in `plates.js` fino alla v0.46.0: `readings.length < minVotes` richiede tre **letture**, non tre letture **concordi**. Con una sorgente che produce una stringa diversa a ogni fotogramma — un orologio in sovrimpressione, per esempio — il voto passa sempre, perché le letture sono tante mentre l'accordo è nullo. La condizione corretta è `best.count < minVotes`. Il codice attuale conta i voti concordi **e** scarta i risultati che non corrispondono a nessun formato reale di targa.
