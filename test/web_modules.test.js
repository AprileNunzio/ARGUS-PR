...
function checkSyntax(file) {
    return new Promise((resolve) => {
        execFile(process.execPath, ['--check', file], (error, _stdout, stderr) => {
            resolve(error ? { file, detail: String(stderr).split('\n').slice(0, 4).join(' ').trim() } : null);
        });
    });
}

test('ogni modulo web e sintatticamente valido', async () => {
    const files = walk(WEB_ROOT);
    const failures = [];

    for (let index = 0; index < files.length; index += CONCURRENCY) {
        const batch = files.slice(index, index + CONCURRENCY);
        const outcomes = await Promise.all(batch.map(checkSyntax));
    ...
    assert.deepEqual(
        failures,
        [],
        failures.map((entry) => `${entry.file}: ${entry.detail}`).join('\n')
    );
});
