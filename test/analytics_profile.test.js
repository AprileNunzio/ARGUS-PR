...
test('disableUnsupported spegne solo la capacita senza modello', () => {
    const entries = mergeProfile([
        { capability: 'vehicle', enabled: true, engineId: 'yolox_nano', threshold: 0.4 },
        { capability: 'plate', enabled: true, engineId: 'plate_crnn', threshold: 0.35 }
    ]);

    const degraded = disableUnsupported(entries, ['text_recognition_crnn_en']);
    const byId = new Map(degraded.map((entry) => [entry.capability, entry]));

    assert.equal(byId.get('vehicle').enabled, true);
    assert.equal(byId.get('plate').enabled, false);
    assert.equal(byId.get('plate').blockedBy, 'models');
    assert.deepEqual(blockedCapabilities(entries, degraded), ['plate']);
});

test('disableUnsupported propaga la dipendenza alle capacita figlie', () => {
    const entries = mergeProfile([
        { capability: 'face_detect', enabled: true, engineId: 'yunet', threshold: 0.6 },
        { capability: 'face_recognize', enabled: true, engineId: 'sface', threshold: 0.363 }
    ]);

    const degraded = disableUnsupported(entries, ['face_detection_yunet']);
    const byId = new Map(degraded.map((entry) => [entry.capability, entry]));

    assert.equal(byId.get('face_detect').enabled, false);
    assert.equal(byId.get('face_recognize').enabled, false);
    assert.deepEqual(blockedCapabilities(entries, degraded).sort(), ['face_detect', 'face_recognize']);
});

test('un modello mancante per le targhe non spegne il rilevamento veicoli', () => {
    const entries = mergeProfile([
        { capability: 'vehicle', enabled: true, engineId: 'yolox_nano', threshold: 0.4 },
        { capability: 'plate', enabled: true, engineId: 'plate_crnn', threshold: 0.35 }
    ]);
    ...
    assert.equal(needsWorker(degraded), true);
    assert.ok(acceptedClasses(degraded).has('car'));
    assert.equal(buildWorkerProfile(degraded, { yolox_nano: 'yolox_nano.onnx' }).tasks.objects.enabled, true);
    assert.equal(buildWorkerProfile(degraded, { yolox_nano: 'yolox_nano.onnx' }).tasks.plates.enabled, false);
});

test('disableUnsupported non tocca il profilo quando i modelli ci sono', () => {
    const entries = mergeProfile([{ capability: 'vehicle', enabled: true, engineId: 'yolox_nano', threshold: 0.4 }]);
    assert.equal(disableUnsupported(entries, []), entries);
});

test('fra piu capacita oggetti attive il worker carica il motore piu capace', () => {
    const entries = mergeProfile([
        { capability: 'person', enabled: true, engineId: 'yolox_nano', threshold: 0.4 },
        { capability: 'vehicle', enabled: true, engineId: 'yolox_tiny', threshold: 0.4 },
        { capability: 'animal', enabled: true, engineId: 'yolox_nano', threshold: 0.45 }
    ]);

    const objects = buildWorkerProfile(entries, MODEL_FILES).tasks.objects;

    assert.equal(objects.engine, 'yolox_tiny');
    assert.equal(strongestObjectEntry(entries).capability, 'vehicle');
});

test('il motore piu capace vince a prescindere dall ordine delle capacita', () => {
    const entries = mergeProfile([
        { capability: 'vehicle', enabled: true, engineId: 'yolox_nano', threshold: 0.4 },
        { capability: 'animal', enabled: true, engineId: 'yolox_tiny', threshold: 0.45 }
    ]);

    assert.equal(buildWorkerProfile(entries, MODEL_FILES).tasks.objects.engine, 'yolox_tiny');
});

test('la soglia degli oggetti resta la piu permissiva fra le capacita attive', () => {
    const entries = mergeProfile([
        { capability: 'person', enabled: true, engineId: 'yolox_nano', threshold: 0.7 },
        { capability: 'vehicle', enabled: true, engineId: 'yolox_tiny', threshold: 0.3 }
    ]);

    assert.equal(buildWorkerProfile(entries, MODEL_FILES).tasks.objects.threshold, 0.3);
});
