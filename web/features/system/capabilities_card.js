...
function commandBlock(command) {
    ...
    const button = el('button', {
        ...
        title: t('system.copiaNegliAppunti', 'Copia negli appunti')
    }, [icon('crop'), el('span', { textContent: t('system.copia', 'Copia') })]);
    ...
    button.addEventListener('click', async () => {
        ...
        setTimeout(() => button.replaceChildren(icon('crop'), el('span', { textContent: t('system.copia', 'Copia') })), 1800);
    });
...
function listOrDash(values, tone = 'info') {
    if (!Array.isArray(values) || values.length === 0) {
        return el('span', { className: 'xrow__hint', textContent: t('system.nessuno', 'nessuno') });
    }
...
export function capabilitiesBody(report) {
    if (!report) {
        return [el('span', { className: 'xrow__hint', textContent: t('system.rilevamentoDelleCapacitaNon', 'Rilevamento delle capacita non disponibile.') })];
    }
    ...
    return [
        ...
        el('div', { className: 'spec-grid' }, [
            el('div', { className: 'spec' }, [
                el('span', { className: 'spec__k', textContent: t('system.piattaforma', 'Piattaforma') }),
            ...
            report.platform.model ? el('div', { className: 'spec' }, [
                el('span', { className: 'spec__k', textContent: t('system.macchina', 'Macchina') }),
                el('span', { className: 'spec__v', textContent: report.platform.model })
            ]) : null,
            el('div', { className: 'spec' }, [
                el('span', { className: 'spec__k', textContent: t('system.ffmpeg', 'ffmpeg') }),
                el('span', { className: 'spec__v', textContent: report.video.ffmpeg.available ? report.video.ffmpeg.version : 'non disponibile' })
            ]),
            el('div', { className: 'spec' }, [
                el('span', { className: 'spec__k', textContent: t('system.runtimeAI', 'Runtime AI') }),
                el('span', { className: 'spec__v', textContent: report.ai.available ? 'operativo' : (report.ai.reason ?? 'non disponibile') })
            ])
        ].filter(Boolean)),

        optionRow({
            title: t('system.acceleratoriDiDecodificaVer', 'Acceleratori di decodifica verificati'),
            ...
            control: el('div', { className: 'stack stack--tight' }, [
                el('div', { className: 'row row--tight' }, [
                    el('span', { className: 'xrow__hint', textContent: t('system.compilati', 'compilati:') }),
                    listOrDash(report.video.accelerators.compiled, 'info')
                ]),
                el('div', { className: 'row row--tight' }, [
                    el('span', { className: 'xrow__hint', textContent: t('system.usabili', 'usabili:') }),
        ...
        optionRow({
            title: t('system.encoderDisponibili', 'Encoder disponibili'),
        ...
        optionRow({
            title: t('system.providerDiInferenza', 'Provider di inferenza'),
        ...
        devices.length > 0 ? optionRow({
            title: t('system.dispositiviVideoDelKernel', 'Dispositivi video del kernel'),
        ...
        modules.length > 0 ? optionRow({
            title: t('system.moduliCodecDelKernel', 'Moduli codec del kernel'),
        ...
        report.suggestions.length > 0
            ? el('div', { className: 'stack stack--tight' }, [
                el('span', { className: 'xrow__title', textContent: t('system.cosaPuoiAbilitareSuQuesta', 'Cosa puoi abilitare su questa macchina') }),
                el('span', { className: 'xrow__hint', textContent: t('system.aRGUSPRNonApplicaNullaDa', 'ARGUS-PR non applica nulla da solo: qui trovi cosa e disponibile e il comando esatto, la scelta resta tua.') }),
                ...report.suggestions.map(suggestionRow)
            ])
            : el('p', { className: 'xcard__note' }, [
                icon('check'),
                el('span', { textContent: t('system.nessunaMiglioriaRilevataLa', 'Nessuna miglioria rilevata: la macchina sta gia usando tutto cio che espone.') })
            ])
    ].filter(Boolean);
}
