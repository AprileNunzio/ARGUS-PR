...
export function renderBust(canvas, mesh, options = {}) {
    ...
    const project = (vertices) => {
        const view = vertices.map((vertex) => rotate([vertex[0], vertex[1] - MODEL_CENTER_Y, vertex[2]], yaw, pitch, roll));
    ...
    const bust = project(mesh.vertices);
    const surface = options.surface && Array.isArray(options.surface.triangles)
        ? { ...project(options.surface.vertices), confidence: options.surface.confidence }
        : null;

    const facets = [];
    for (const quad of mesh.quads) {
        const a = bust.view[quad[0]];
        const b = bust.view[quad[1]];
        const c = bust.view[quad[2]];
        const normal = faceNormal(a, b, c);
        if (normal[2] <= 0.02) continue;

        const depth = (a[2] + b[2] + c[2] + bust.view[quad[3]][2]) / 4;
        facets.push({ indices: quad, geometry: bust, normal, depth, measured: 0 });
    }

    if (surface) {
        for (const triangle of options.surface.triangles) {
            const a = surface.view[triangle[0]];
            const b = surface.view[triangle[1]];
            const c = surface.view[triangle[2]];
            const normal = faceNormal(a, b, c);
            if (normal[2] <= 0.02) continue;

            const weights = surface.confidence ?? null;
            const measured = weights
                ? (weights[triangle[0]] + weights[triangle[1]] + weights[triangle[2]]) / 3
                : 1;
            facets.push({
                indices: triangle,
                geometry: surface,
                normal,
                depth: (a[2] + b[2] + c[2]) / 3,
                measured: Math.max(0, Math.min(1, measured))
            });
        }
    }
    ...
    for (const facet of facets) {
        ...
        const points = facet.geometry.screen;
        ctx.beginPath();
        const first = points[facet.indices[0]];
        ctx.moveTo(first[0], first[1]);
        for (let i = 1; i < facet.indices.length; i += 1) {
            const point = points[facet.indices[i]];
        ...
        if (showFacets) {
            const channel = (index) => {
                const base = theme.facetLow[index] + (theme.facetHigh[index] - theme.facetLow[index]) * mix;
                if (facet.measured <= 0) return Math.round(base);
                const real = theme.faceLow[index] + (theme.faceHigh[index] - theme.faceLow[index]) * mix;
                return Math.round(base + (real - base) * facet.measured);
            };
