...
import { buildFaceSurface } from './face_mesh_surface.js';
...
function getSurface(params) {
    ...
    if (!Array.isArray(mesh)) return null;
    if (surfaceCache.has(mesh)) return surfaceCache.get(mesh);

    const surface = buildFaceSurface(mesh, params?.meshConfidence ?? null);
    surfaceCache.set(mesh, surface);
    return surface;
}
...
export function createFace3DCanvas(params = {}, width = 180, height = 180) {
    ...
    const surface = getSurface(params);
    ...
    renderBust(canvas, getMesh(params, detail), {
        ...
        roll: params?.roll ?? 0,
        surface,
        landmarks: surface ? null : alignMeshToHead(meshFromLandmarks(params?.mesh)),
...
export function createFace3DViewer(params = {}, { width = 420, height = 480 } = {}) {
    ...
    const surface = getSurface(params);
    const landmarks = surface ? null : alignMeshToHead(meshFromLandmarks(params?.mesh));
