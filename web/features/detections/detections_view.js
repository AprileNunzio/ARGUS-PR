...
import { openLightbox } from '/assets/lightbox.js';
...
export async function renderDetectionsView({ api, session }) {
    ...
    async function loadEvents() {
        ...
        const rows = events.map((ev) => {
            ...
            const caption = [
                label,
                ev.plateText ? `targa ${ev.plateText}` : null,
                ev.cameraName ?? ev.cameraId,
                dateStr
            ].filter(Boolean).join(' · ');

            const snapshotImg = snapshotUrl
                ? el('button', {
                    type: 'button',
                    className: 'detection-thumbnail__button',
                    title: "Apri l'immagine a grandezza piena",
                    onclick: () => openLightbox({
                        src: snapshotUrl,
                        title: label,
                        caption,
                        downloadName: `${ev.className}-${new Date(ev.startedAt).toISOString().replace(/[:.]/g, '-')}.jpg`
                    })
                }, [
                    el('img', { src: snapshotUrl, className: 'detection-thumbnail', alt: caption, loading: 'lazy' }),
                    el('span', { className: 'detection-thumbnail__zoom' }, [icon('search')])
                ])
                : el('span', { className: 'detection-thumbnail detection-thumbnail--empty' }, [icon('image')]);

            const durationMs = ev.endedAt ? new Date(ev.endedAt).getTime() - new Date(ev.startedAt).getTime() : 0;
            const durationText = durationMs >= 1000
                ? `${(durationMs / 1000).toFixed(durationMs < 60000 ? 1 : 0)} s in campo`
                : 'istantaneo';

            return el('div', { className: 'device-row detection-row' }, [
                snapshotImg,
                el('div', { className: 'stack stack--tight grow' }, [
                    ...
                    el('div', { className: 'row row--tight' }, [
                        el('strong', { textContent: ev.cameraName ?? ev.cameraId }),
                        el('span', { className: 'section__hint', textContent: durationText }),
                        el('span', { className: 'section__hint', textContent: `confidenza ${confPct}` }),
                        ev.trackId ? el('span', { className: 'section__hint mono', textContent: `traccia ${ev.trackId.slice(0, 8)}` }) : null
                    ])
                ]),
                seekBtn
            ]);
