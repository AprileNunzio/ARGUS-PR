...
const ROUTE_LABELS = {
    ...
    live: 'shell.live',
    'wall-settings': 'shell.wallSettings',
    archive: 'shell.archive',
    cameras: 'shell.cameras',
    detections: 'shell.detections',
    access: 'shell.access',
    people: 'shell.people',
    automation: 'shell.automation',
    floorplan: 'shell.floorplan',
    settings: 'shell.settings',
    storage: 'shell.storage',
    system: 'shell.system',
    updates: 'shell.updates',
    maintenance: 'shell.maintenance',
    datetime: 'shell.datetime',
    audio: 'shell.audio',
    users: 'shell.users'
};
...
function visibleRoutes() {
    ...
    return Object.fromEntries(
        Object.entries(ROUTES)
            .filter(([, route]) => !route.permission || granted.includes(route.permission))
            .map(([name, route]) => [
                name,
                { ...route, label: ROUTE_LABELS[name] ? t(ROUTE_LABELS[name]) : route.label }
            ])
    );
}
...
window.addEventListener('argus:locale-changed', async () => {
    if (state.session) {
        await showApp();
    }
});
