...
export function disableUnsupported(entries, missingModelNames = []) {
    if (!Array.isArray(missingModelNames) || missingModelNames.length === 0) return entries;
    const missing = new Set(missingModelNames);

    const next = entries.map((entry) => {
        if (entry.enabled !== true) return { ...entry };
        const engine = findEngine(entry.engineId);
        const blocked = (engine?.models ?? []).some((model) => missing.has(model));
        return blocked ? { ...entry, enabled: false, blockedBy: 'models' } : { ...entry };
    });

    return applyDependencies(next);
}

export function blockedCapabilities(before, after) {
    const stillActive = new Set(after.filter((entry) => entry.enabled === true).map((entry) => entry.capability));
    return before
        .filter((entry) => entry.enabled === true && !stillActive.has(entry.capability))
...
function objectEntries(entries) {
    return activeEntries(entries).filter((entry) => findCapability(entry.capability)?.task === 'objects');
}

export function strongestObjectEntry(entries) {
    const candidates = objectEntries(entries);
    if (candidates.length === 0) return null;

    return candidates.reduce((best, entry) => {
        const bestCost = findEngine(best.engineId)?.cost ?? 0;
        const cost = findEngine(entry.engineId)?.cost ?? 0;
...
export function buildWorkerProfile(entries, modelFiles = {}) {
    ...
    const objectEntry = strongestObjectEntry(entries)
    ...
    const objectThreshold = Math.min(...objectEntries(entries)
        .map((entry) => entry.threshold)
        .concat(objectEntry ? [objectEntry.threshold ?? 0.4] : []));

    const objectMinSize = Math.min(...objectEntries(entries)
        .map((entry) => entry.minSize ?? 0)
        .concat(objectEntry ? [objectEntry.minSize ?? 0] : []));
    ...
    return {
        tasks: {
            objects: {
                ...
                minSize: Number.isFinite(objectMinSize) ? objectMinSize : 0,
                classes
            },
            faces: {
