    ...
    update(detections = [], timestamp = Date.now()) {
        ...
        for (const match of matches) {
            ...
            if (det.confidence > track.maxConfidence) {
                ...
                if (det.snapshotBase64) track.bestSnapshot = det.snapshotBase64;
            } else if (!track.bestSnapshot && det.snapshotBase64) {
                track.bestSnapshot = det.snapshotBase64;
            }
        ...
        for (const detIndex of unmatchedDets) {
            ...
            const track = {
                ...
                bestSnapshot: det.snapshotBase64 ?? null,
                startedAt: timestamp,
                endedAt: timestamp,
                hits: 1,
                misses: 0,
