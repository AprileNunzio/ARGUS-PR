...
import { analysisSizeFor } from './analysis_geometry.js';
...
export function createVisionProcess({ camera, ffmpegPath, pythonBin, dataDir, modelsDir, performanceSettings = {}, sourceSize = null, workerProfile = null, onDetections, onError }) {
    ...
    const analysis = analysisSizeFor(sourceSize?.width ?? camera.captureWidth, sourceSize?.height ?? camera.captureHeight);
    stats.analysisSize = `${analysis.width}x${analysis.height}`;
    ...
    function start() {
        ...
        if (!local) {
            ...
            ffmpegArgs.push(
                '-an',
                '-vf', `fps=${analysisFps},scale=${analysis.width}:${analysis.height}`,
                '-f', 'rawvideo',
                '-pix_fmt', 'bgr24',
                'pipe:1'
            );
        ...
        const workerArgs = [
            ...
            '--frame-width', String(analysis.width),
            '--frame-height', String(analysis.height)
        ];
        if (workerProfile) {
