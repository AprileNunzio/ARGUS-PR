...
const EARLY_FAILURE_MS = 15000;
const AUDIO_FALLBACK_AFTER = 3;
...
export class Recorder {
    ...
    start() {
        ...
        const args = buildRecordArgs(
            input,
            {
                ...
                listingPath,
                withAudio: camera.audioEnabled !== false && !this.audioDisabled
            }
        );
    ...
    recycle(reason) {
        const lived = this.startedAt ? Date.now() - this.startedAt : 0;
        this.teardown();
        if (this.stopped) return;

        if (lived > 0 && lived < EARLY_FAILURE_MS) this.earlyFailures += 1;
        else this.earlyFailures = 0;

        if (!this.audioDisabled && this.earlyFailures >= AUDIO_FALLBACK_AFTER) {
            this.audioDisabled = true;
            this.earlyFailures = 0;
            log.warn('recording falls back to video only', { camera: this.cameraId, reason });
        }
